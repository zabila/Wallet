    ...
    public void ConfigureState(StateMachine<BotState, BotTrigger> stateMachine, UserSession userSession) {
        stateMachine.Configure(State)
            ...
            .OnEntryFromAsync(BotTrigger.AmountEntered, async transition => {
                ...
                await botClient.SendMessage(userSession.ChatId, string.Format(TelegramBot.ExpenseAmountEnteredStateDefinition_ConfigureState_You_entered_amount__0__for_category__1_, amount, userSession.StateData[BotState.ExpenseCategorySelected]));
