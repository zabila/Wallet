...
public class AccountController(ISender sender) : ControllerBase
{
    ...
    public async Task<IActionResult> GetAccountByTelegramId([FromRoute] Guid userid, long telegramid)
    {
        var account = await sender.Send(new GetAccountByTelegramUserIdQuery(userid, telegramid));
        return Ok(account);
    }
}
